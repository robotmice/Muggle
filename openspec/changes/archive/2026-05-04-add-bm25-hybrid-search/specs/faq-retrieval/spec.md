...
The system SHALL support retrieving relevant FAQ sections using hybrid search that combines dense vector similarity with BM25 sparse vector keyword matching across multiple languages.
...
- **AND** it MUST merge and deduplicate results from both language searches
- **AND** it MUST return the top N relevant FAQ entries (header, text, and lang_tag) from the merged results
