...
from muggle.shared.constants import STR_PROMPT_VALIDATION
...
class ValidationNode:
    ...
    def __call__(self, state: WorkflowState, config: RunnableConfig) -> dict:
        system_prompt = self.prompt_registry.get_system_prompt(
            STR_PROMPT_VALIDATION, variables={"threshold": str(self.threshold)}
        )
        ...
        return {
            ...
            "attempt_count": state.attempt_count + 1 if not result.pass_validation else state.attempt_count,
        }
